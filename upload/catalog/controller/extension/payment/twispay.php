...
class ControllerExtensionPaymentTwispay extends Controller {
    ...
    public function index() {
        ...
        if ($order_info) {
            ...
            if(!empty($testMode)) {
                ...
                $postfields['siteId'] = $this->config->get('payment_twispay_staging_site_id');
                $privateKEY = $this->config->get('payment_twispay_staging_site_key');
            } else {
                $action = 'https://secure.twispay.com';
                $postfields['siteId'] = $this->config->get('payment_twispay_live_site_id');
                $privateKEY = $this->config->get('payment_twispay_live_site_key');
            }
    ...
    {
        $apiKey = (!empty($this->config->get('payment_twispay_testMode'))) ? $this->config->get('payment_twispay_staging_site_key') : $this->config->get('payment_twispay_live_site_key');
    ...
    public function callback() {
        ...
        if(empty(trim($this->config->get('payment_twispay_redirect_page')))){
            $page_to_redirect = $baseurl. '/index.php?route=checkout/success';
        } else {
            $page_to_redirect = trim($this->config->get('payment_twispay_redirect_page'));
        ...
        $this->log_file = $this->config->get('payment_twispay_logs').'/twispay_log.txt';
                ...
                if(!empty($result)){
                    ...
                    if(empty($transaction)){
                        ...
                        if ($order_info && !$order_info['invoice_no']) {
                            ...
                            $order_status_id = (!empty($this->config->get('payment_twispay_order_status_id'))) ? $this->config->get('payment_twispay_order_status_id') : $this->config->get('config_order_status_id');
    ...
    public function s2s(){
        $this->log_file = $this->config->get('payment_twispay_logs').'/twispay_s_log.txt';
        if(file_exists($this->log_file) && filesize($this->log_file) > 2097152){
            @file_put_contents($this->log_file, PHP_EOL.PHP_EOL);
        }
        if(!empty($_POST)){
